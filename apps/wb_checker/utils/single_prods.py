    ...
    def updating_plus_notification(self, price_of_detail, volume):
        ...
        if self.current_detail_to_check.volume != volume: #по количеству постоянные изменения - просто пишу в бд без уведомлений (пока что)
            if self.current_detail_to_check.volume >= 10 and volume < 10:
                ...
                self.notifications_to_save.append(Notification(text=f'<i>🛒WildBerries</i> <br> <b>📦{self.current_detail_to_check.product.name}</b> <br>  ❗️<b>Менее 10 штук в наличии!</b> Успейте купить :) <br> 💵<b>Текущая цена:</b> {self.current_detail_to_check.latest_price}₽{detailed_text}',
    ...
    def make_notification(self, price_of_detail):
        ...
        if abs(self.current_detail_to_check.latest_price - price_of_detail) > self.current_detail_to_check.author.notification_discount_price or abs(int((self.current_detail_to_check.latest_price-price_of_detail)/(self.current_detail_to_check.latest_price/100))) > self.current_detail_to_check.author.notification_discount:
            if self.current_detail_to_check.latest_price > price_of_detail and self.current_detail_to_check.author.pricedown_notification is True:
                ...
                self.notifications_to_save.append(Notification(text=f'<i>🛒WildBerries</i> <br> <b>📦{self.current_detail_to_check.product.name}</b> <br> 🟢 Цена <b>упала</b> на <b>{self.current_detail_to_check.latest_price - price_of_detail} ₽</b>! (-{int((self.current_detail_to_check.latest_price-price_of_detail)/(self.current_detail_to_check.latest_price/100))}%) <br> 💵<b>Текущая цена:</b> {price_of_detail}₽{detailed_text}',
                ...
                self.current_detail_to_check.last_notified_price = price_of_detail
                self.notifications_to_save.append(Notification(text=f'<i>🛒WildBerries</i> <br> <b>📦{self.current_detail_to_check.product.name}</b> <br> 🔴 Цена <b>поднялась</b> на <b> {price_of_detail - self.current_detail_to_check.latest_price} ₽</b>! (+{int((price_of_detail-self.current_detail_to_check.latest_price)/(self.current_detail_to_check.latest_price/100))}%) <br> 💵<b>Текущая цена:</b> {price_of_detail}₽{detailed_text}',
...
class AvaliabilityUpdater:
    ...
    def enable_product(self, price_of_detail, volume):
        ...
        self.notifications_to_save.append(Notification(text=f'<i>🛒WildBerries</i> <br> <b>📦{self.current_detail_to_check.product.name}</b> <br> <b> ✅ Появился в наличии! </b> Успейте купить! <br> 💵<b>Текущая цена:</b> {self.current_detail_to_check.latest_price}₽{detailed_text}',
