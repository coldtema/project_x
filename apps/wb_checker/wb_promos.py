...
# from .models import WBProduct, WBSeller, WBPrice, WBCategory, WBBrand, WBPromotion
...
#         self.promo_object, self.promo_was_not_in_db = self.get_object_of_promo()
#         self.promo_api_url = self.construct_promo_api_url()
#         self.total_products = self.get_total_products_in_catalog()
#         self.number_of_pages = self.get_number_of_pages_in_catalog()
#         #делаем кэш на промо которое приходит, всех брендов, селлеров
...
#         self.get_catalog_of_promo()
#         self.get_repetitions_catalog_promo()
#         self.add_all_to_db()
...
#             self.promo_api_url = re.sub(pattern=r'page=\d+\&', repl=f'page={elem}&', string=self.promo_api_url)
#             while True:
#                 try:
...
#         WBProduct.enabled_products.bulk_create([self.promo_object], update_conflicts=True, unique_fields=['wb_id'], update_fields=['name']) #можно переписать в create_or_update
#         self.promo_object.save()
...
#             elem.promotion = self.promo_object
#         WBProduct.objects.bulk_update(self.repetition_products_to_change, ['promotion'])
...
#         if not potential_promo_object:
#             clear_shard_key = '/'.join(json_data['shardKey'].split('/')[1:])
#             return WBPromotion(name=json_data['name'],
#                             wb_id=json_data['id'],
#                             main_url=self.promo_url,
#                             shard_key=clear_shard_key,
#                             query=json_data['query'])
#         else:
#             return potential_promo_object
...
#         sorting = 'popular'
#         addons = []
#         if '?' in clear_promo_url:
#             addons = clear_promo_url.split('?')[1]
#             addons = addons.split('&')
#             for elem in addons:
#                 if 'sort' in elem:
