...
def all_price_list(request):
    if request.method == 'POST':
        ...
        if product_form.is_valid():
            ...
            new_product = Product.objects.create(name=product_data['name'], shop=product_data['shop'], latest_price=product_data['price_element'], url=request.POST.get('url'), author=Author.objects.get(id=2))
